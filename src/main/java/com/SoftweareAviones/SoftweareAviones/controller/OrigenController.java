...
import com.SoftweareAviones.SoftweareAviones.services.OrigenService;
...
public class OrigenController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Origen> editarOrigen(@PathVariable Integer id, @Valid @RequestBody Origen origen) {
        try {
            origen.setId_origen(id);
            Origen editado = origenService.actualizarOrigen(id, origen);
